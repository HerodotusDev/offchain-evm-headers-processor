...
PATH_CAIRO_PROGRAMS  = "src/single-chunk-processor"
CAIRO_FILES = [f for f in listdir(PATH_CAIRO_PROGRAMS) if isfile(join(PATH_CAIRO_PROGRAMS, f)) if f[-6:]=='.cairo']
LIB_CAIRO_FILES = ["src/cairo_mrr/"+f for f in listdir("src/cairo_mmr") if isfile(join("src/cairo_mmr", f)) if f[-6:]=='.cairo']
LIB_CAIRO_FILES += ["src/"+f for f in listdir("src/") if isfile(join("src/", f)) if f[-6:]=='.cairo']
