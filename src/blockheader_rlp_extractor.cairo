...
from src.extract_from_rlp import extract_data, extractElement, jumpOverElement, to_list
from src.types import Keccak256Hash, Address, IntsSequence
...
func decode_transactions_root{range_check_ptr}(block_rlp: IntsSequence) -> (res: Keccak256Hash) {
    ...
    let (local data: IntsSequence) = extract_data(
        4 + 32 + 1 + 32 + 1 + 20 + 1 + 32 + 1, 32, block_rlp
    );
...
func decode_difficulty{range_check_ptr}(block_rlp: IntsSequence) -> (res: felt) {
    ...
    let (local difficulty_rlp_element: IntsSequence) = extract_data(
        rlp_items[DIFFICULTY].dataPosition, rlp_items[DIFFICULTY].length, block_rlp
    );
...
func decode_block_number{range_check_ptr}(block_rlp: IntsSequence) -> (res: felt) {
    ...
    let (local block_number_rlp_element: IntsSequence) = extract_data(
        rlp_items[BLOCK_NUMBER].dataPosition, rlp_items[BLOCK_NUMBER].length, block_rlp
    );
...
func decode_gas_limit{range_check_ptr}(block_rlp: IntsSequence) -> (res: felt) {
    ...
    let (local gas_limit_rlp_element: IntsSequence) = extract_data(
        rlp_items[GAS_LIMIT].dataPosition, rlp_items[GAS_LIMIT].length, block_rlp
    );
...
func decode_gas_used{range_check_ptr}(block_rlp: IntsSequence) -> (res: felt) {
    ...
    let (local gas_used_rlp_element: IntsSequence) = extract_data(
        rlp_items[GAS_USED].dataPosition, rlp_items[GAS_USED].length, block_rlp
    );
...
func decode_timestamp{range_check_ptr}(block_rlp: IntsSequence) -> (res: felt) {
    ...
    let (local timestamp_rlp_element: IntsSequence) = extract_data(
        rlp_items[TIMESTAMP].dataPosition, rlp_items[TIMESTAMP].length, block_rlp
    );
...
func decode_base_fee{range_check_ptr}(block_rlp: IntsSequence) -> (res: felt) {
    ...
    let (local base_fee_rlp_element: IntsSequence) = extract_data(
        rlp_items[BASE_FEE].dataPosition, rlp_items[BASE_FEE].length, block_rlp
    );
