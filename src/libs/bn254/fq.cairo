...
namespace fq {
    ...
    func double{range_check_ptr}(a: BigInt3*) -> BigInt3* {
        alloc_locals;
        let (__fp__, _) = get_fp_and_pc();
        ...
        %{
            ...
            p_limbs = [getattr(ids, 'P'+str(i)) for i in range(ids.N_LIMBS)]
            sum_limbs = [getattr(getattr(ids, 'a'), 'd'+str(i)) + getattr(getattr(ids, 'a'), 'd'+str(i)) for i in range(ids.N_LIMBS)]
            sum_unreduced = sum([sum_limbs[i] * BASE**i for i in range(ids.N_LIMBS)])
            sum_reduced = [sum_limbs[i] - p_limbs[i] for i in range(ids.N_LIMBS)]
            has_carry = [1 if sum_limbs[0] >= BASE else 0]
            for i in range(1,ids.N_LIMBS):
        ...
        if (needs_reduction != 0) {
            ...
            local res: BigInt3 = BigInt3(
                (-P0) + a.d0 + a.d0 - cb_d0 * BASE,
                (-P1) + a.d1 + a.d1 + cb_d0 - cb_d1 * BASE,
                (-P2) + a.d2 + a.d2 + cb_d1,
            );
            ...
            local res: BigInt3 = BigInt3(
                a.d0 + a.d0 - cb_d0 * BASE, a.d1 + a.d1 + cb_d0 - cb_d1 * BASE, a.d2 + a.d2 + cb_d1
            );
            assert [range_check_ptr] = BASE_MIN_1 - res.d0;
            assert [range_check_ptr + 1] = BASE_MIN_1 - res.d1;
            assert [range_check_ptr + 2] = P2 - res.d2;
    ...
    func add_rc{range_check_ptr, round_constants: BigInt3*}(
        a: BigInt3*, arr_index: felt
    ) -> BigInt3* {
        alloc_locals;
        let (__fp__, _) = get_fp_and_pc();
        let b: BigInt3 = round_constants[arr_index];
        // %{ print("ADDRC:", ids.arr_index, ids.b.d0, ids.b.d1, ids.b.d2) %}
    ...
    func square{range_check_ptr}(a: BigInt3*) -> BigInt3* {
        // a must be reduced mod P and in their unique representation
        // a = a0 + a1*B + a2*B², with 0 <= a0, a1, a2 < B and 0 < a < P
        alloc_locals;
        let (__fp__, _) = get_fp_and_pc();
        local q: BigInt3;
        ...
        %{
            ...
            for i in range(ids.N_LIMBS):
                a+=as_int(getattr(ids.a, 'd'+str(i)),PRIME) * ids.BASE**i
                b+=as_int(getattr(ids.a, 'd'+str(i)),PRIME) * ids.BASE**i
                p+=getattr(ids, 'P'+str(i)) * ids.BASE**i
                a_limbs[i]=as_int(getattr(ids.a, 'd'+str(i)),PRIME)
                b_limbs[i]=as_int(getattr(ids.a, 'd'+str(i)),PRIME)
                p_limbs[i]=getattr(ids, 'P'+str(i))
        ...
        tempvar diff_d0 = q.d0 * P0 + r.d0 - a.d0 * a.d0;
        tempvar diff_d1 = q.d0 * P1 + q.d1 * P0 + r.d1 - 2 * a.d0 * a.d1;  // a.d0 * a.d1 - a.d1 * a.d0;
        tempvar diff_d2 = q.d0 * P2 + q.d1 * P1 + q.d2 * P0 + r.d2 - 2 * a.d0 * a.d2 - a.d1 * a.d1;
        tempvar diff_d3 = q.d1 * P2 + q.d2 * P1 - 2 * a.d1 * a.d2;
        ...
        assert q.d2 * P2 - a.d2 * a.d2 + carry3 = 0;

        tempvar range_check_ptr = range_check_ptr + 10;
        return &r;
    }

    func add_c{range_check_ptr}(a: BigInt3*, b: BigInt3*) -> BigInt3* {
        alloc_locals;
    ...
    func sub_c{range_check_ptr}(a: BigInt3*, b: BigInt3*) -> BigInt3* {
        alloc_locals;
    ...
    func mul_c{range_check_ptr}(a: BigInt3*, b: BigInt3*) -> BigInt3* {
        alloc_locals;
