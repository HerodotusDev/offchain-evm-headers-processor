...
from src.types import IntsSequence, RLPItem
...
func getElement{range_check_ptr}(rlp: IntsSequence, position: felt) -> (res: RLPItem) {
    ...
    let (local data: IntsSequence) = extract_data{range_check_ptr=range_check_ptr}(
        position, 1, rlp
    );
    ...
    if (le_191 == 1) {
        ...
        let (local bytes: IntsSequence) = extract_data{range_check_ptr=range_check_ptr}(
            position + 1, lengthOfLength, rlp
        );
...
func extract_list_values{range_check_ptr}(
    rlp: IntsSequence, rlp_items: RLPItem*, rlp_items_len: felt
) -> (res: IntsSequence*, res_len: felt) {
    alloc_locals;
...
func extract_list_values_recursive{range_check_ptr}(
    ...
    let (local data: IntsSequence) = extract_data(
        rlp_items[current_index].dataPosition, rlp_items[current_index].length, rlp
    );
